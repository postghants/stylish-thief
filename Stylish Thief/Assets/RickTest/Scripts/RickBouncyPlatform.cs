...
public class RickBouncyPlatform : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        if (other.gameObject.layer == 6)
        {
            ...
            {
                player.ctx.rb.velocity = directionalForce;
                player.Machine.ChangeState(player.Root.Leaf(), player.Root.airborne);
            }
            else
            {
                player.ctx.rb.velocity = player.ctx.rb.velocity - new Vector3(0, player.ctx.rb.velocity.y, 0) + directionalForce;
                player.Machine.ChangeState(player.Root.Leaf(), player.Root.airborne);
            }
        }
    }
    ...
    private void OnTriggerEnter(Collider other)
    {
        if (other.gameObject.layer == 6)
        {
            ...
            player.ctx.rb.velocity = Vector3.up * 50;
